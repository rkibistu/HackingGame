...
public class TasksController : MonoBehaviour
{
    ...
    [Tooltip("This is the the text for the displayed up and right on the screen")]
    [SerializeField]
    private TextMeshProUGUI _currentObjectiveText;
    [Tooltip("This is the panel displayed up and right on the screen")]
    [SerializeField]
    private GameObject _currentObjectivePanel;
    [Tooltip("This is the text used inside the igname menu to see the description of the tasks")]
    [SerializeField]
    ...
    public void ActivateTask(string id)
    {
        foreach (var task in _tasks.tasks)
        {
            ...
            taskRow.OnTaskRowClicked.AddListener(HandleTaskRowClicked);
            taskRow.Init(task);
            task.row = taskRow;
    ...
    private void HandleTaskRowClicked(string id)
    {
        foreach (var task in _journalRows.Values)
        {
            if (task.id == id)
            {
                UpdateObjectiveDescriptionPanel(task.description);
                return;
            }
            foreach (var step in task.steps)
            {
    ...
    //Default value is the description of current objective
